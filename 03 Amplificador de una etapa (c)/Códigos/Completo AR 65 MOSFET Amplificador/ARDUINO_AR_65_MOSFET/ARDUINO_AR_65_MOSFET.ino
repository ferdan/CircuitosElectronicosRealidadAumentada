 ...
 * numero_entradas_analogicas -> es el numero de entradas analogicas que se leen del circuito
 ...
 *   buffer_promediador -> Es el buffer en el que se almacenan las muestras leidas
...
/**
 * ---------------------------------------------------------------------------------------------------------
 * num_curvas_id_vds -> Indica el numero de curvas para las que se tomaran muestras para graficas id vs Vds
 * num_curvas_parametros -> Indica el numero de curvas para las que se tomaran muestras para calcular parametros hibridos
 * num_curvas_pequena_senal -> Indica el numero de curvas para las que se tomaran muestras para graficar curvas del transistor
 *                             como amplificador en pequena senal
 * ---------------------------------------------------------------------------------------------------------
 */
const int num_curvas_id_vds = 5;
const int num_curvas_parametros = 1;
const int num_curvas_pequena_senal = 1;
...
/**
 * Se miden cinco curvas para graficar Id vs vds. Posteriormente se toma una curva para Id vs Vgs y una ultima curva para el amplificador en pequeña señal
 * 
 */
void loop() {
  ...
  if (millis()-lastTime > sampleTime) {
    ...
    if(numero_curva<num_curvas_id_vds) // ¿Se toman muestras de las primeras "num_curvas_id_vds" curvas para Id vs Vds?
    {
      int valor;
      // --- 5 Curvas para Id vs Vds [MOSFET] ---    
      valor = map(numero_curva,0,5,108,122); //probar tambien con 116 - 132
    ...
    else if(numero_curva<num_curvas_id_vds+num_curvas_parametros+num_curvas_id_vds+num_curvas_pequena_senal)
    {
      // --- 1 Curva para pequena senal ---
      establecer_voltaje_Vdd(255);
      barrer_voltaje_Vgg_pequena_senal(2.31,0.075);
    }
    else
    {
      numero_curva = 0;
...
/**
 * Actualiza Vgg con diferentes valores de una senoidal con amplitud pequena
 ...
 * 
 */
void barrer_voltaje_Vgg_pequena_senal(double Voffset, double Amplitud_pico){
  for (int i_muestra=0; i_muestra<250; i_muestra+=5){
    sine = byte(255*((Voffset/Vdd)+((Amplitud_pico/(Vdd))*sin((1000/sampleTime)*TWO_PI*i_muestra/(numMuestras-1)))));
